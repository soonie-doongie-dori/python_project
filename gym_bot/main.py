...
def message_handler(message):
    global gym
    global exercise
    if re.fullmatch('[0-9]+\s[0-9]+', message.text.strip()) and gym != '' and exercise != '':
        ...
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton('Новое упражнение', callback_data='/new_exercise'),
...
def callback_query(call):
    ...
    # Создаем меню подтверждения добавления подхода
        ...
        markup.add(InlineKeyboardButton('Добавить подход', callback_data='/new_rep'),
                   InlineKeyboardButton('Выбрать другое упражнение', callback_data='/new_exercise'))
        bot.send_message(
            call.message.chat.id,
            f'Выбрано упражнение: {exercise}',
            reply_markup=markup
        )
    # Выводим информацию по добавлению подхода
        ...
        bot.send_message(
            call.message.chat.id,
            f'Зал: {gym}. Упражнение: {exercise}.\nВведите данные подхода в формате: минут повторений'
        )
